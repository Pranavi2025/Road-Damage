...
def UploadDataset():
    ...
    if os.path.exists("model/X1.npy"):
        ...
        for roots, dirs, directory in os.walk(path):
            for j in range(len(directory)):
                tree = ET.parse(roots+"/"+directory[j])
                root = tree.getroot()
                img_name = root.find('filename').text
                arr = img_name.split("_")
                img = cv2.imread("RDD2022_China_Drone/images/"+img_name)
                if img is not None:
                    ...
                    for item in root.findall('object'):
                        ...
                        if index < 12:
                            xmin, ymin, xmax, ymax = normalizeBoxes([xmin, ymin, xmax, ymax], width, height)
                            boxes[index] = xmin
                            index = index + 1
                            boxes[index] = ymin
                            index = index + 1
